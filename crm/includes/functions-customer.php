...
function erp_crm_get_life_stages_dropdown_raw( $label = [] ) {
    ...
    $life_stages = [
        'customer'    => __( 'Project Jalan / Selesai 100%', 'wp-erp' ),
        'lead'        => __( 'PKS / PO signed 80%', 'wp-erp' ),
        'opportunity' => __( 'Negoisasi 60%', 'wp-erp' ),
        'subscriber'  => __( 'Send Proposal 40%', 'wp-erp' ),
        'subscribero' => __( 'Send Compro / Meeting 20%', 'wp-erp' )
    ];
...
function erp_crm_customer_get_company( $postdata ) {
    ...
    if ( isset( $postdata['id'] ) && empty( $postdata['id'] ) ) {
        return new WP_Error( 'no-ids', __( 'Kontak tidak ditemukan', 'erp' ) );
    }
...
function erp_crm_get_customer_feeds_nav() {
    return apply_filters( 'erp_crm_customer_feeds_nav', [
        ...
        'new_note' => [
            'title' => __( 'Topik / Issue / Diskusi', 'erp' ),
            'icon'  => '<i class="fa fa-file-text-o"></i>'
        ],

        'email' => [
            'title' => __( 'Kirim Email', 'erp' ),
            'icon'  => '<i class="fa fa-envelope-o"></i>'
        ],

        'log_activity' => [
            'title' => __( 'Aktifitas Hari Ini', 'erp' ),
            'icon'  => '<i class="fa fa-list"></i>'
        ],

        'schedule' => [
            'title' => __( 'Next Schedule Action', 'erp' ),
            'icon'  => '<i class="fa fa-calendar-check-o"></i>'
        ],

        'tasks' => [
            'title' => __( 'Create Tugas', 'erp' ),
