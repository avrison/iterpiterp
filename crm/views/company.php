...
<div class="wrap erp-crm-customer erp-crm-customer-listing" id="wp-erp">
    ...
    <h2><?php _e( 'Listing Company', 'erp' ); ?>
        <?php if ( current_user_can( 'erp_crm_add_contact' ) ): ?>
            <a href="#" @click.prevent="addContact( 'company', '<?php _e( 'Tambahkan Company', 'erp' ); ?>' )" id="erp-company-new" class="erp-contact-new add-new-h2" data-type="company" title="<?php _e( 'Tambahkan Company', 'erp' ); ?>"><?php _e( 'Tambahkan Company', 'erp' ); ?></a>
        <?php endif; ?>

        <a href="#" @click.prevent="addSearchSegment()" id="erp-contact-search-segmen" class="erp-search-segment add-new-h2" v-text="( showHideSegment ) ? '<?php _e( 'Sembunyikan Pencarian Segment Segment', 'erp' ); ?>' : '<?php _e( 'Tambah Pencarian Segment', 'erp' ); ?>'"></a>
    </h2>
