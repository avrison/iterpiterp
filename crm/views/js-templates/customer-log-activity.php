...
<div id="log-activity">
    <p>
        <select name="log_type" v-model="feedData.log_type" id="log-type" class="erp-left">
            <option value=""><?php _e( '-- Pilih type --', 'erp' ) ?></option>
            <option value="call"><?php _e( 'Telepon', 'erp' ) ?></option>
            <option value="meeting"><?php _e( 'Meeting', 'erp' ) ?></option>
            <option value="penawaran"><?php _e( 'Kirim Penawaran', 'erp' ) ?></option>
            <option value="lainnya"><?php _e( 'Lainnya', 'erp' ) ?></option>
        </select>
    ...
    <p v-if="feedData.log_type == 'meeting'">
        <select name="selected_contact" id="erp-crm-activity-invite-contact" v-model="feedData.inviteContact" v-selecttwo="feedData.inviteContact" class="select2" multiple="multiple" style="width: 100%" data-placeholder="Select agents...">
